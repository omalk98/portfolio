import { ReactNode, useLayoutEffect, useState } from "react";
import HeaderNav from "@/components/header";
import Footer from "@/components/footer";

export default function Layout({ children }: { children: ReactNode }) {
  // document.getElementById("top")?.clientHeight || 0;
  const [height, setHeight] = useState(0);
  useLayoutEffect(() => {
    setTimeout(() => {
      const element = document.getElementById("top");
      setHeight(element?.clientHeight || 0);
    }, 50);
  }, []);
  return (
    <div className='min-h-screen overflow-hidden'>
      <HeaderNav className='mx-auto max-w-[1200px]' />

      <div className='relative dark:bg-grid-white/[0.2] bg-grid-black/[0.2]'>
        <div className='h-screen absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]' />
        <div style={{transform: `translateY(${height}px)`}} className='absolute pointer-events-none inset-0 flex items-center justify-center dark:bg-black bg-white [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]' />
        <main className='relative mx-auto pt-5'>{children}</main>

        <Footer className='relative' />
      </div>
    </div>
  );
}
